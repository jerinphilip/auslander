...
public class WhisperEngine implements IWhisperEngine {
  ...
  private String runInference(float[] inputData) {
    ...
    //        try {
    //            byte[] bytes =
    // Files.readAllBytes(Paths.get("/data/user/0/com.example.tfliteaudio/files/mel_spectrogram.bin"));
    ...
    for (int i = 0; i < outputLen; i++) {
      ...
      if (token < mWhisperUtil.getTokenEOT()) {
        ...
        if (token == mWhisperUtil.getTokenTranscribe()) Log.d(TAG, "It is Transcription...");

        if (token == mWhisperUtil.getTokenTranslate()) Log.d(TAG, "It is Translation...");
